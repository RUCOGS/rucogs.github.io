...
def parse_existing_article_infos(text: str) -> List[ArticleInfo]:
    ...
    for matched_article_info in matched_article_infos:
        authors = []
        for matched_author in re.findall(
            PARSE_STRING_ARRAY_REGEX, matched_article_info[5]
        ):
            authors.append(matched_author)
        tags = []
        for matched_tag in re.findall(
            PARSE_STRING_ARRAY_REGEX, matched_article_info[6]
        ):
...
existing_article_infos_dict = parse_existing_article_infos_into_dict(
    original_typescript_file_text
)
...
ARTICLE_INFO_INSERTION_POINT_REGEX = (
    r"([\s\S]*?export const BlogPageArticles = \[)(?:[\s\S]*)(\][\s\S]*)"
)

insertion_point_matches = re.findall(
    ARTICLE_INFO_INSERTION_POINT_REGEX, original_typescript_file_text
)
