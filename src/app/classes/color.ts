export class Color {
  ...
  shade(light: number): Color {
    if (light < -1 || light > 1) throw new Error("'light' must be between -1 and 1.");
  ...
  static fromHex(hex: string): Color | null {
    ...
    if (hex.charAt(0) == '#') hex = hex.substr(1, hex.length - 1);
  ...
  static getConstrastColor(color: Color): Color {
    return this.useDarkColor(color)
      ? this.fromVarOrDefault('var(--dark)', this.Types.cyan)
      : this.fromVarOrDefault('var(--light)', this.Types.cyan);
  }
  ...
  static getContrastColorFromText(colorText: string): Color {
    ...
    if (matches !== null) {
      ...
      if (contrast !== null) return contrast;
    }
