...
export class FileUtils {
  static byteSizeToString(byteSize: number) {
    if (byteSize > DataSize.GB) return `${this.byteTo(byteSize, DataSize.GB)} GB`;
    else if (byteSize > DataSize.MB) return `${this.byteTo(byteSize, DataSize.MB)} MB`;
    else if (byteSize > DataSize.KB) return `${this.byteTo(byteSize, DataSize.KB)} KB`;
