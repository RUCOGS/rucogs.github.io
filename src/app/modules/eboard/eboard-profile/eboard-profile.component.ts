...
import { getCurrentAcademicYear } from '@src/app/utils/user-utils';
...
export class EboardProfileComponent implements OnInit {
  ...
  hasRoles() {
    const lookupYear = getCurrentAcademicYear();
  ...
  getRoles() {
    ...
    if (this.eBoard.terms && this.eBoard.terms.length > 0)
      return this.eBoard.terms.find((x) => x?.year === lookupYear)?.roles?.map((x) => x?.roleCode as RoleCode) ?? [];
    return [];
  }
