...
export class EboardTabComponent {
  ...
  constructor(private uiMessage: UIMessageService, private backend: BackendService, private dialog: MatDialog) {}
  ...
  async onEditTerm(term: PartialDeep<EBoardTerm>) {
    const success = await firstValueFrom(
      this.dialog
        .open(EditEboardTermDialogComponent, {
  ...
  async onCreateEBoard() {
    const success = await firstValueFrom(
      this.uiMessage.confirmDialog('Are you sure you want to create an e-board profile?'),
    );
    if (!success) return;
    const result = await firstValueFrom(
      this.backend.withAuth().mutate({
  ...
  async onEditEBoard() {
    const success = await firstValueFrom(
      this.dialog
        .open(EditEboardDialogComponent, {
  ...
  async onDeleteEBoard() {
    const success = await firstValueFrom(
      this.uiMessage.confirmDialog('Are you sure you want to delete the e-board profile?'),
    );
    if (!success) return;
    const result = await firstValueFrom(
      this.backend.withAuth().mutate({
